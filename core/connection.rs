...
impl Connection {
    ...
    fn prepare_with_origin(
        ...
        // Temporarily force SQLite dialect so the parser and planner don't
        // hide internal tables like sqlite_schema when in Postgres mode.
    ...
    fn list_user_tables(self: &Arc<Self>, schema_name: Option<&str>) -> Result<Vec<String>> {
        ...
        let sql = match schema_name {
            Some(name) => format!(
                "SELECT name FROM \"{name}\".sqlite_schema WHERE {filter}"
            ),
